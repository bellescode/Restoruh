/* ============================================================
   ...
   Drop this in: your GitHub repo at app/routes/_index.jsx
   This replaces the existing _index.jsx entirely.
   ============================================================ */
...
export async function loader({context}) {
  ...
  const teas   = storefront.query(COLLECTION_QUERY, {variables: {handle: 'teas',   first: 6}});
  const blends = storefront.query(COLLECTION_QUERY, {variables: {handle: 'blends', first: 5}});
  return defer({teas, blends});
}
          ...
          {cats.map(c => (
            <Link
              ...
              style={{
                ...
                border: `1.5px solid ${c.dark ? 'transparent' : 'rgba(30,58,44,.12)'}`,
                textDecoration: 'none',
              }}
            >
              ...
              <span style={{
                ...
                fontWeight: 700,
                letterSpacing: 2,
                background: c.dark ? 'rgba(201,162,74,.2)' : 'rgba(30,58,44,.08)',
              ...
              <p style={{fontSize: 15, color: c.dark ? 'var(--sage)' : 'var(--muted)', lineHeight: 1.6}}>
                {c.sub}
              </p>
            </Link>
          ))}
  ...
  return (
    <section className="mission-band">
      <div className="container">
        ...
        <p style={{textAlign: 'center', fontSize: 13, fontWeight: 600, letterSpacing: 3, color: 'var(--sage)'}}>
...
function ProductCard({product}) {
  ...
  return (
    ...
    >
      <div style={{aspectRatio: '1', overflow: 'hidden', background: 'rgba(30,58,44,.05)'}}>
        {image
      ...
      <div style={{padding: '20px'}}>
        <p style={{fontSize: 10, fontWeight: 700, letterSpacing: 2, color: 'var(--sage)', marginBottom: 6, textTransform: 'uppercase'}}>
          {product.productType || 'Herbal'}
        </p>
        <h3 style={{fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 600, color: 'var(--green)', marginBottom: 8}}>
          {product.title}
        </h3>
        <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12}}>
          <span className="badge-soon">Pre-Order</span>
          {price && (
            <span style={{fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 600, color: 'var(--green)'}}>
              <Money data={price} />
            </span>
          )}
        </div>
...
/* --- TEA PREVIEW ----------------------------------------- */
function TeaPreview({teasPromise}) {
  return (
      ...
      <div className="container">
        <div style={{display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 40}}>
          <div>
            ...
            <h2 style={{fontFamily: 'var(--font-display)', fontSize: 'clamp(26px,4vw,40px)', fontWeight: 600, color: 'var(--green)'}}>
              The Tea Library
            </h2>
          </div>
          <Link to="/collections/teas" className="btn-ghost" style={{fontSize: 13, padding: '9px 22px'}}>
            View all teas
          </Link>
        </div>
        <div className="scroll-row">
          {teasPromise?.collection?.products?.nodes?.map(p => (
...
/* --- BLEND PREVIEW --------------------------------------- */
function BlendPreview({blendsPromise}) {
  return (
    <section className="section" style={{background: 'var(--paper)'}}>
      <div className="container">
        <div style={{display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 40}}>
          ...
          <Link to="/collections/blends" className="btn-ghost" style={{fontSize: 13, padding: '9px 22px'}}>
            View all blends
          </Link>
        </div>
        <div className="product-grid">
          {blendsPromise?.collection?.products?.nodes?.map(p => (
            <ProductCard key={p.id} product={p} />
          ))}
        </div>
      </div>
        ...
        <div style={{display: 'grid', gap: 48, gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', alignItems: 'center'}}>
          <div>
            ...
            <p style={{fontSize: 16, color: 'var(--sage)', lineHeight: 1.7, marginBottom: 32}}>
              Faith-based wellness research. Published studies, tested tradition, and practical guidance for every household. Completely free.
            </p>
  ...
  return (
    <section className="newsletter-section">
      <div className="container">
        ...
        <p style={{fontSize: 15, color: 'var(--muted)', lineHeight: 1.6, marginBottom: 32, maxWidth: 480, margin: '0 auto 32px'}}>
          Our teas and blends are in final development. Join the list for first access and the founding member offer.
        </p>
        {done ? (
          <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, background: 'rgba(63,125,79,.15)', padding: '16px 24px', borderRadius: 12, maxWidth: 400, margin: '0 auto'}}>
          ...
          <div className="newsletter-form">
            ...
            />
            <button onClick={submit} className="btn-gold">Notify me</button>
          </div>
        )}
...
export default function Homepage() {
  ...
  return (
    <>
      ...
      <ProductCategories />
      <MissionBand />
      <TeaPreview   teasPromise={teas}     />
      <BlendPreview blendsPromise={blends} />
      <DirectoryCallout />
      <Newsletter />
    </>
  );
