...
export default function App({ initialSection="home", initialId=null }){
  const [user,  setUser]  = useState(null);
  const [authLoaded, setAuthLoaded] = useState(false);
  const [view,  setView]  = useState(
    initialSection === "home"
      ? {name:"home"}
      : {name: initialSection, id: initialId || undefined}
  );
  ...
  const DIR_URL = {
    home:           "/directory",
    herbs:          "/directory/herbs",
    "master-blend": "/directory/herbs?view=master-blend",
    herb:           (v) => `/directory/herbs?id=${v.id}`,
    systems:        "/directory/systems",
    system:         (v) => `/directory/systems?id=${v.id}`,
    ailments:       "/directory/ailments",
    ailment:        (v) => `/directory/ailments?id=${v.id}`,
    "gut-health":   "/directory/gut-health",
    "oral-health":  "/directory/oral-health",
    toxicity:       "/directory/toxicity",
    nutrients:      "/directory/nutrients",
    nutrition:      "/directory/nutrition",
    cleansing:      "/directory/cleansing",
    movement:       "/directory/movement",
    plan:           "/directory/family-plan",
    prepared:       "/directory/prepared",
    area:           (v) => `/directory/area/${v.id}`,
  };
  const go = (v) => {
    setView(v);
    setQuery("");
    ...
    const raw = DIR_URL[v.name];
    if (raw) navigate(typeof raw === "function" ? raw(v) : raw);
  };
  ...
  return(
    <div style={{background:"var(--cream)",color:"var(--ink)"}}>
      ...
      {/* Compact search bar -- sits below site sub-nav */}
      <div style={{background:"var(--green-deep)",padding:"10px 24px",borderBottom:"1px solid rgba(201,162,74,.15)"}}>
        <div style={{maxWidth:900,margin:"0 auto",display:"flex",alignItems:"center",gap:10,background:"rgba(245,240,227,.1)",borderRadius:12,padding:"8px 14px"}}>
          <Search size={16} color="var(--sage)"/>
          <input value={query} onChange={(e)=>setQuery(e.target.value)}
            placeholder="Search herbs, ailments, or body systems..."
            style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:14,color:"var(--cream)",fontFamily:"var(--font-body)"}}/>
          {query&&<button onClick={()=>setQuery("")} style={{background:"none",border:"none",cursor:"pointer"}}><X size={14} color="var(--sage)"/></button>}
        </div>
      </div>
